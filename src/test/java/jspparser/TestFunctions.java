...
public class TestFunctions extends BaseTest{
	...
	public void setup(){
		...
		request.setAttribute("arr", new String[]{"The","rain","in","Spain"});
		request.setAttribute("list", Arrays.asList(new String[]{"The","rain","in","Spain"}));
	}
	...
	public void test_contains(){
		renderer.render(page, session);
		String text = new String(baos.toByteArray());
		assertTrue(text.contains("Condition 3"));
		assertFalse(text.contains("Condition 4"));
	}
	...
	public void test_containsIgnoreCase(){
		renderer.render(page, session);
		String text = new String(baos.toByteArray());
		assertTrue(text.contains("Condition 5"));
		assertFalse(text.contains("Condition 6"));
	}
	...
	public void test_endsWith(){
		renderer.render(page, session);
		String text = new String(baos.toByteArray());
		assertTrue(text.contains("Condition 7"));
		assertFalse(text.contains("Condition 8"));
	}

	@Test
	public void test_escapeXml(){
		//TODO: escapeXml(...) will work as advertised. however, if I <c:out value="fn:escapeXml(...)"/> this will double-encode
		//the result. verify how "standard" JSP does it.
		request.setAttribute("xml", "<howdy/>");
		renderer.render(page, session);
		String text = new String(baos.toByteArray());
		assertTrue(text.contains("escapeXml &lt;howdy/&gt;"));
	}
	...
	public void test_indexOf(){
		renderer.render(page, session);
		String text = new String(baos.toByteArray());
		assertTrue(text.contains("indexOf 4"));
	}
